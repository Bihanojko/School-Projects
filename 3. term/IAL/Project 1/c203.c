...
*/
    if (q == NULL)		// chybny vstupni parametr
		queueError(QERR_INIT);
    else
    {
		for (int i = 0; i < (QUEUE_SIZE); i++)	// do kazdeho prvku pole
		    q->arr[i] = '*';			// uloz znak '*'
			q->f_index = 0;				// pocatecni indexy nastavime na 0
			q->b_index = 0;				// seznam je zatim prazdny
    }
}
...
*/
    if (queueEmpty(q))		// pokud je fronta prazdna -> queueError()
		queueError(QERR_FRONT);
    else
		*c = q->arr[q->f_index];	// ulozi znak do promenne
}
...
*/
    if (queueEmpty(q))			// pokud je fronta prazdna -> queueError()
		...
		q->f_index = nextIndex(q->f_index);	// nastavi index prvniho prvku na nasledujici prvek
}
...
*/
    ...
    {
		queueFront(q, c);	// do promenne c uloz prvni znak
		queueRemove(q);		// odstrani prvni prvek (posune ukazatel)
    }
}
...
*/
    if (queueFull(q))		// pokud je fronta plna -> queueError()
		queueError(QERR_UP);
    else
    {
		q->arr[q->b_index] = c;			// ulozi znak na prvni volne misto ve fronte
		q->b_index = nextIndex(q->b_index);	// posune index prvniho volneho prvku na nasledujici prvek
    }
}
